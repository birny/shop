...
function displayProducts($product, $max_length = 0) {

	$html = '
	<div class="product">
	    <p>'.date('d-m-Y H:i:s', strtotime($product['date'])).' par <a href="#">'.$product['name'].'</a></p>
	    <blockquote>
	      <p>';

	if ($max_length > 0) {
		$html .= cutString($product['description'], $max_length, '... <a href="product.php?id='.$product['id'].'">Lire la suite</a>');
	} else {
...
function getProductRating($rating = 0.0) {
	$html = '';
	for ($i=0; $i < 5; $i++) { 
